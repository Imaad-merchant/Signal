...
export default function TransactionsView({ data, onChange = () => {} }) {
  ...
  return (
    <>
      <Card
        title="Transactions"
        right={<span className="text-[11px] text-gray-500">{rows.length} · net {fmtMoney(total)}</span>}
      >
        <Filters filters={filters} onChange={(f) => { setFilters(f); setShown(PAGE); }} accounts={accounts} />

        <div className="mt-3 flex items-center justify-between gap-2">
          <button onClick={toggleAll} className="inline-flex items-center gap-1.5 text-[11px] text-gray-400 hover:text-gray-200">
            {allSelected ? <CheckSquare className="h-3.5 w-3.5" /> : <Square className="h-3.5 w-3.5" />}
            {allSelected ? "Clear" : "Select all"}
          </button>
          <AddAndImport accounts={accounts} onChange={onChange} setMsg={setMsg} />
        </div>
        {msg && <p className="mt-1 text-[11px] text-cyan-300">{msg}</p>}

        {selected.size > 0 && (
          <div className="mt-2 flex flex-wrap items-center gap-1.5 rounded-lg border border-cyan-400/20 bg-cyan-400/5 p-2">
            <span className="text-[11px] text-cyan-200">{selected.size} selected</span>
            <select
              defaultValue=""
              onChange={(e) => { if (e.target.value) { bulk({ category: e.target.value }); e.target.value = ""; } }}
              className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-[11px] text-gray-300 outline-none"
            >
              <option value="" className="bg-[#0e1015]">Set category…</option>
              {CATEGORIES.map((c) => <option key={c} value={c} className="bg-[#0e1015]">{c}</option>)}
            </select>
            <button onClick={() => bulk({ ignored: true })} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1 text-[11px] text-gray-300 hover:border-orange-400/40">
              <Ban className="h-3 w-3" /> Ignore
            </button>
            <button onClick={bulkDelete} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1 text-[11px] text-rose-300 hover:border-rose-400/40">
              <Trash2 className="h-3 w-3" /> Delete
            </button>
            {busy === "bulk" && <Loader2 className="h-3.5 w-3.5 animate-spin text-gray-400" />}
          </div>
        )}
        ...
        <div className="mt-2 flex flex-col">
          {visible.map((t) => {
            ...
            return (
              <div key={t.id} className={`group flex items-center gap-2 rounded-lg px-1 py-1.5 ${isSel ? "bg-cyan-400/5" : "hover:bg-white/[0.03]"}`}>
                <button onClick={() => toggle(t.id)} className="p-1 text-gray-600 hover:text-gray-300">
                  {isSel ? <CheckSquare className="h-3.5 w-3.5 text-cyan-300" /> : <Square className="h-3.5 w-3.5" />}
                </button>
                <button onClick={() => setOpenTx(t)} className="flex min-w-0 flex-1 items-center gap-2.5 text-left">
                  <span className="w-9 shrink-0 text-[10px] text-gray-500">{String(t.date || "").slice(5)}</span>
        ...
        {rows.length > shown && (
          <button onClick={() => setShown((n) => n + PAGE)} className="mt-3 w-full rounded-lg border border-white/10 py-2 text-[11px] text-gray-300 hover:border-cyan-400/40">
...
function AddAndImport({ accounts, onChange, setMsg }) {
  ...
  if (adding) {
    return (
      <div className="flex w-full flex-wrap items-center gap-1.5">
        <input value={date} onChange={(e) => setDate(e.target.value)} type="date" className="rounded-lg border border-white/10 bg-white/5 px-2 py-1.5 text-[11px] text-gray-300 outline-none" />
        ...
        <select value={cat} onChange={(e) => setCat(e.target.value)} className="rounded-lg border border-white/10 bg-white/5 px-2 py-1.5 text-[11px] text-gray-300 outline-none">
  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={() => setAdding(true)} className="inline-flex items-center gap-1 text-[11px] text-cyan-300 hover:text-cyan-200"><Plus className="h-3 w-3" /> Add</button>
