...
export default function ImportActivitiesDialog({ open, onOpenChange, onImported }) {
  ...
  // idle | uploading | extracting | review | creating | done | error | syncing
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {/* UPLOAD STEP */}
        {(status === "idle" || status === "uploading" || status === "extracting" || status === "error" || status === "syncing") && (
          <div className="space-y-4 mt-1">
            {status === "idle" && (
              <Button
                onClick={handleGoogleSync}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl h-10 gap-2 mb-3"
              >
            ...
            {status === "syncing" && (
              <div className="flex flex-col items-center py-6 gap-3">
                ...
                <p className="text-sm text-gray-600">Syncing your Google Calendar...</p>
              </div>
            )}
            {!file ? (
              <div
